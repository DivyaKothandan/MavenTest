...
public class SulekhaHomePage extends Initializer{
	...
	public static void validateSearch() throws InterruptedException, IOException {
		...
		ReadExcel obj = new ReadExcel("C:/Users/hp/workspace/Sulekha/src/test/java/inputReader/SulekhaData.xlsx");
		
		
		//to write in Excel...
		WriteExcel wb = new WriteExcel("C:/Users/hp/workspace/Sulekha/src/test/java/inputReader/SulekhaData.xlsx");
		...
		{
			//Writing data in excel at sheet name 
			wb.setCellData("data", "List", k, options.get(j).getText());
			k++;
		...
		{
			if(options.get(i).getText().equalsIgnoreCase(searchItem))
			{
				options.get(i).click();
				Thread.sleep(5000);
				break;
